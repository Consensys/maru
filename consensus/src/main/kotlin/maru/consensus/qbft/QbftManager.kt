...
private const val eventQueueSize = 1000 // TODO: Make this configurable
private val roundExpiry = Duration.of(1, ChronoUnit.SECONDS) // TODO: Make this configurable
...
class QbftManager(
  ...
  fun start() {
    ...
    val qbftBlockCreatorFactory = QbftBlockCreatorFactoryImpl(executionLayerManager, proposerSelector, staticValidatorProvider, beaconChain)

    val clock = Clock.systemUTC()
    val bftExecutors = BftExecutors.create(metricsSystem, BftExecutors.ConsensusType.QBFT)
    val bftEventQueue = BftEventQueue(eventQueueSize)
    val roundTimer = RoundTimer(bftEventQueue, roundExpiry, bftExecutors)
    val forksSchedule = ForksSchedule<BftConfigOptions>(emptyList()) // TODO create besu forksSchedule from maru forksSchedule
    val blockTimer = BlockTimer(bftEventQueue, forksSchedule, bftExecutors, clock)
    val finalState = QbftFinalStateAdapter(localAddress, nodeKey,
      validatorProvider, proposerSelector, validatorMulticaster, roundTimer, blockTimer, qbftBlockCreatorFactory,
      clock
    )
  }
}
